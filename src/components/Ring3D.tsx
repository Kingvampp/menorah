...
interface Menorah3DProps {
  gemstoneColor?: string;
  metalColor?: string;
}

export default function Menorah3D({ gemstoneColor = '#4A90E2', metalColor = '#D4AF37' }: Menorah3DProps) {
  ...
  // Load your custom menorah model
  const { scene } = useGLTF('/models/menorah_render.glb');
      ...
      cloned.traverse((child) => {
        if (child instanceof THREE.Mesh) {
          ...
          const isDiamond = child.name.toLowerCase().includes('diamond') || 
                           ...
                           child.name.toLowerCase().includes('crystal') ||
  ...
  useEffect(() => {
    const canvas = document.getElementById('menorah-canvas') as HTMLCanvasElement;
    if (!canvas) return;

    const handleMouseDown = (event: MouseEvent) => {
      // Only respond if the event target is this specific canvas
  ...
  // No entrance animation - start at final position
  useEffect(() => {
    setEntranceProgress(1); // Set to final position immediately
  }, []);

  // Globe-like animation system
  useFrame((state) => {
    if (menorahRef.current) {
      // Static position - no entrance animation
      menorahRef.current.position.x = 0;
      menorahRef.current.position.y = 0;
      menorahRef.current.position.z = 0; // Keep centered
      ...
      // Apply rotations
      menorahRef.current.rotation.x = rotationX;
      menorahRef.current.rotation.y = rotationY;
      menorahRef.current.rotation.z = 0; // Keep Z rotation minimal
      
      // Keep materials fully visible
      menorahRef.current.traverse((child) => {
  ...
  return (
    <group>
      ...
      {/* Your custom menorah model */}
      {clonedScene && (
        <group ref={menorahRef} dispose={null}>
          <primitive 
            object={clonedScene} 
            scale={2.2} 
            position={[0, 0.2, 0]}
